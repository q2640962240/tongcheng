...
37. **`tryRefresh` 递归自死锁：续期端点自己的 401 会永久挂起整个 app（2026-09-08 S5 修，P0）** —— 症状极具误导性：**骨架屏永久转圈、不跳登录页、不弹 toast、console 零 JS 错误、刷新也没用**，用户被彻底锁在 app 外。
    - **机制（三行代码构成的循环）**：`request.js` 的 401 分支原先**无条件** `await tryRefresh()`；`tryRefresh()` 置 `refreshLock` 后调 `authApi.refresh(rt)`，而它走的是**同一个** `request()` 包装器 → 它自己的 401 又进 401 分支 → 递归 `await tryRefresh()` → 命中 `if (refreshLock) return refreshLock`，而这个 lock **正是当前在等它的那个 promise** → 循环 await，永不 settle。
    - **连带后果链（每一环都让排查更难）**：① `refreshLock = null` 永远执行不到 → 该页后续所有 401 也一起挂死；② `kickToLogin()` 与 `reject()` 永远到不了 → token 不清、不跳登录、无 toast；③ 所有 `await` 该请求的调用方永久挂起 → z-paging 的 `complete()` 从不被调用 → 骨架屏不消失。**因为 ②，`removeToken()` 没跑过，于是留下了下面这条指纹。**
    - **★ 诊断指纹：查 `removeToken()` 该清的三个键还在不在 ★** —— `auth.js` 的 `removeToken()` 只清 `companion_token` / `companion_refresh_token` / `companion_user`。三者**仍在** = `kickToLogin` 主体没执行 = promise 没 settle = 挂起，而不是「跑了但失败」。这是区分「静默挂起」与「逻辑走错分支」最快的一刀，比读代码或加日志都快。（localStorage 里另有 `token`/`user_token`/`uni_token`/`admin_token` 等历史遗留键，`removeToken` 不管它们，别拿它们判断。）
    - **★ A/B 单变量实证 ★**：只删 `companion_refresh_token` 一个键 → `tryRefresh()` 在 `if (!rt) return false` 早退 → 同一页面立刻正常跳 `#/pages/login/login`、token 清空、`/api/auth/refresh` 请求数为 **0**、登录页渲染完好。一次改动同时证明「死锁在 refresh 递归上」和「其余链路都是好的」。
    - **触发面：100% 的老用户** —— 坑点 33 的 S3 轮换了 `JWT_SECRET`/`JWT_REFRESH_SECRET`，使所有存量 refresh token 失效。**新用户不受影响**：无任何 token 时 `requireLogin()` 直接走弹窗、根本不进 401 路径；只有 access token 而没有 refresh token 时 `tryRefresh` 早退。**所以「新装的能用、老用户全废」正是这个 bug 的指纹**，别误判成账号数据问题。
    - **不是 D8（z-paging）回归** —— 死锁在 `request.js`，D8 一行没碰它。旧版 transactions 的 `guard()` 包的是同一个永不 settle 的 promise，`finally { loading.value = false }` 同样跑不到，一样会永久骨架屏。D8 的冒烟检查只是**让它暴露**了。
    - **修法（2 处代码、3 次编辑，刻意最小）**：给 `request()` 加 `noRefreshRetry` 选项（与既有 `silent` 完全同机制，`post(url,data,extra)` 会把 `extra` 展开进 options），`authApi.refresh` 传 `{ noRefreshRetry: true }`，401 分支改成 `noRefreshRetry ? false : await tryRefresh()`。于是刷新端点自身的 401 直接落到 `kickToLogin` + `reject`，被 `tryRefresh` 的 `catch(_)` 吞掉后返回 false，外层同样 `kickToLogin`（12s 防抖吃掉重复，只跳一次）+ reject，链路终止。**不要顺手重构 `tryRefresh` 的锁机制或给 `kickToLogin` 加逻辑** —— 那是全局 HTTP 层，影响面远大于这个 bug。
    - **规则：刷新端点自己绝不能触发刷新。** 任何「全局 401 拦截 + 自动续期」的设计都必须给续期端点开一扇后门，否则续期失败时必然自指。同理适用于将来新增的 `logout`、`re-login` 之类端点——评估标准是「这个请求失败时再去续期有没有意义」。
